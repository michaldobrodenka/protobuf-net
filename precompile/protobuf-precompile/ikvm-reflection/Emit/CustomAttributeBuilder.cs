	...
	public sealed class CustomAttributeBuilder
	{
		...
		private sealed class BlobWriter
		{
			...
			private void GetTypeName(StringBuilder sb, Type type, bool isTypeParam)
			{
				...
				bool includeAssemblyName = type.ExportAssembly != assembly && (!v1 || type.ExportAssembly != type.Module.universe.Mscorlib);
				...
				if (includeAssemblyName)
				{
					...
					if (isTypeParam)
					{
						sb.Append(type.ExportAssembly.FullName.Replace("]", "\\]")).Append(']');
					}
					else
					{
						sb.Append(type.ExportAssembly.FullName);
					}
				}
			}
