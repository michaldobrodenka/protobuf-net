...
namespace IKVM.Reflection.Reader
{
	...
	sealed class ModuleReader : Module
	{
		...
		private readonly Dictionary<TypeName, LazyForwardedType> forwardedTypes = new Dictionary<TypeName, LazyForwardedType>();
		...
		private sealed class LazyForwardedType
		{
			...
			internal LazyForwardedType(int index, string fullName, Assembly assembly)
			{
				this.index = index;
			...
			internal Type GetType(ModuleReader module)
			{
				...
				else if (type == null)
				{
					...
					type.ForwardingAssembly = Assembly;
				}

				var assemblyFullName = type?.Assembly?.FullName;
				if (!string.IsNullOrEmpty(assemblyFullName) && assemblyFullName.Contains(".Private"))
				{
					Console.WriteLine($"**** LazyForwardedType::GetType {FullName} {assemblyFullName}");
				}
			...
			if (typeDefs == null)
			{
				...
				for (int i = 0; i < ExportedType.records.Length; i++)
				{
					...
					if (implementation >> 24 == AssemblyRefTable.Index)
					{
						...
						var fullName = $"{typeName.Namespace}.{typeName.Name}";
						forwardedTypes.Add(typeName, new LazyForwardedType(i, fullName, this.Assembly));

						// Console.WriteLine($"forwarded {this.Name} {fullName}");
					}
				}
			}
		}
