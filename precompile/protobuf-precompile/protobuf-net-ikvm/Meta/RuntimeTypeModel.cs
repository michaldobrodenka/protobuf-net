...
//#endif
        ...
        public sealed class CompilerOptions
        {
            ...
            /// Returns assembly path of assembly that decided on target framework
            /// </summary>
            public string SetFrameworkOptions(MetaType from)
            {
                if (from == null)
                    throw new ArgumentNullException("from");

                var assembly = Helpers.GetAssembly(@from.Type);
                AttributeMap[] attribs = AttributeMap.Create(from.Model, assembly);
